...
def get_config_local_path():
    ...
    config.unbatch = True
    config.learning_rate = 0.001
    config.optimizer = 'radam' #SGD, adam
    config.weight_decay = 1e-4
    config.momentum = 0.9
    config.dropout = 0.1
    config.batch_size = 8
    ...
    config.skip_connections = True
    config.data_augmentation = False
    config.loss_fnc = 'dice_focal_loss'#'focal' #'dice_focal_loss'#'focal_tversky'#'weighted_crossentropy'#'dice_focal_loss'#'tversky' #crossentropy
    config.decoder_conv_localpath = False
    config.decoder_conv_globalpath = False
